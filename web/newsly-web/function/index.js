...
export const create_tokens = functions.https.onRequest({cors: true}, async (req, res) => {
...
export const checkout = functions.https.onRequest({cors: true}, async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: "https://add-balance-axaeefmbvq-uc.a.run.app?token=" + token + "&amount=" + amount, 
        cancel_url: "https://newsly.studio/"
    })
    res.redirect(301, session.url)
})

export const add_balance = functions.https.onRequest({cors: true}, async (req, res) => {
    const {token, amount} = req.query
